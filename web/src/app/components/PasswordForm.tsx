...
const toggleGroupItemClasses =
	"flex  w-30 py-2 items-center justify-center leading-4 text-mauve11 first:rounded-l last:rounded-r hover:bg-violet3 focus:z-10 focus:outline-none data-[state=on]:bg-violet6 data-[state=on]:bg-border";

export function PasswordForm({ onSave, editEntry, onCancelEdit, defaultType = 'pwd' }: PasswordFormProps) {
  ...
  const [maxLength, setMaxLength] = useState(editEntry?.ruleLimitsMax || 20);
  ...
  const [symbols, setSymbols] = useState(editEntry?.ruleWhitelist?.includes('SYM') || false);

  const [title, setTitle] = useState(editEntry?.title ?? "");
  const [contents, setContents] = useState(editEntry?.contents ?? "");

  const [number, setNumber] = useState(editEntry?.number ?? "");
  const [name, setName] = useState(editEntry?.name ?? "");
  const [exp, setExp] = useState(editEntry?.exp ?? "");
  const [cvc, setCVC] = useState(editEntry?.cvc ?? "");
  const [pin, setPIN] = useState(editEntry?.pin ?? "");
  ...
  const handleSubmit = () => {
    if (type === 'note') {
      onSave({ type: 'note', title: title.trim(), contents: contents.trim() });
      if (!editEntry) {
        setTitle("");
        setContents("");
      }
      ...
      onSave({ type: 'card', title: title.trim(), number, name, exp, pin, cvc });
    } else if (type === 'pwd') {
      ...
      onSave({ type: 'pwd', service: service.trim(), username: username.trim(), password, ruleWhitelist, ruleLimitsMin, ruleLimitsMax });
      if (!editEntry) {
        setService("");
        setUsername("");
        setPassword("");
  ...
  const action = useMemo(() =>{
    return isEditing ? 'Edit' : 'Create New';
  }, [isEditing]);
  const label = useMemo(() =>{
    return `${action} ${type === 'card' ? 'Credit card' :(type === 'note' ? 'Note': 'Password')}`;
  }, [type,action]);
  return (
    <div
      className="rounded-lg p-6 absolute size-full xl:static"
      style={{
    ...
    >
      <div className="relative flex items-center justify-between mb-6">
        ...
        <X className="absolute top-0 right-0 pointer"
            onClick={onCancelEdit} size={24} />
      </div>
      ...
      {!isEditing && <div  className=" mb-4">
		...
		>
			<div className="flex items-center">
				<RadioGroup.Item
					className="size-[25px] cursor-default rounded-full bg-white outline-none"
					value="pwd"
            style={{
              border: "1px solid var(--pm-border)",
            }}
      ...
      <>
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6">
          <div>
            ...
            <input
              type="text"
              placeholder="My bank card"
              className={inputCls}
              style={inputStyle}
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              onFocus={(e) => (e.target.style.borderColor = focusStyle)}
              onBlur={(e) => (e.target.style.borderColor = "var(--pm-border)")}/></div><div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Limit to 16 digits
                const limitedValue = clearValue.substring(0, 16);
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 gap-4 mb-6">
              ...
              onChange={(e) => {
                ...
                // Limit to 16 digits
                const limitedValue = clearValue.substring(0, 4);
                // Add space after every 4 digits
              ...
              onChange={(e) => {
                ...
                // Limit to 16 digits
                const limitedValue = clearValue.substring(0, 3);
            ...
            </div><div className="grid grid-cols-2 md:grid-cols-2 gap-4 mb-6">
            <div><label
              className="block mb-1.5 text-[13px]"
              style={{ color: "var(--pm-text-muted)" }}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6">
          <div>
            ...
            >
              ...
              value={contents}
              onChange={(e) =>  setContents(e.target.value)}/></div></div></>}
      <button
        onClick={handleSubmit}
        disabled={(type === 'pwd' && (!service.trim() || !username.trim())) || (type === 'note' && (!title.trim() || !contents.trim())) }
